...
public class ResponseValidators {
   ...
   * @param expectedHttpMethod   The {@link SzHttpMethod} that was used.
   * @param selfLink             The self link to be expected.
   * @param maxDuration          The maximum duration for the timers in
   *                             nanoseconds.
   */
  public static void validateBasics(SzBasicResponse response,
   ...
   * for the timings in nanoseconds.
   *
   * @param testInfo           Additional test information to be logged with
   *                           failures.
   * @param response           The {@link SzBasicResponse} to validate.
   * @param expectedHttpMethod The {@link SzHttpMethod} that was used.
   * @param selfLink           The self link to be expected.
   ...
   * to upper case and returns a {@link Set} containing all values.
   *
    ...
    if (forceMinimal != null && forceMinimal) {
      ...
      if (expectedFeatureCounts != null) {
        expectedFeatureCounts.entrySet().forEach(entry -> {
          ...
          List<SzEntityFeature> featureValues = entity.getFeatures().get(featureKey);
   ...
   * @param response               The response to validate.
   * @param selfLink               The expected meta data self link.
   * @param withRaw                <tt>true</tt> if requested with raw data,
   *                               <tt>false</tt>
   *                               if requested without raw data and <tt>null</tt>
   *                               if this is
   *                               not being validated.
   * @param withRelated            The {@link SzRelationshipMode} value or
   *                               <tt>null</tt>
   *                               if this aspect is not being validated.
   * @param detailLevel            The {@link SzDetailLevel} describing the level
   *                               of
   *                               detail that was requested for the entity.
   * @param forceMinimal           <tt>true</tt> if requested with minimal data,
   *                               <tt>false</tt> if requested with standard data
   *                               and
   *                               <tt>null</tt> if this aspect is not being
   *                               validated.
   * @param featureMode            The {@link SzFeatureMode} requested or
   *                               <tt>null</tt> if this is not being validated.
   * @param withFeatureStats       <tt>true</tt> if request with feature
   *                               statistics,
   *                               otherwise <tt>false</tt>.
   * @param withInternalFeatures   <tt>true</tt> if request with internal
   *                               features,
   *                               otherwise <tt>false</tt>.
   * @param expectedRecordCount    The number of expected records for the entity,
   *                               or <tt>null</tt> if this is not being
   *                               validated.
   * @param expectedRecordIds      The expected record IDs for the entity to have
   *                               or
   *                               <tt>null</tt> if this is not being validated.
   ...
   *                               type, or <tt>null</tt> if this is not being
   *                               validated.
   * @param duplicateFeatureValues The expected duplicate feature values by
  ...
  public static void validateSearchResponse(
    ...
    for (SzAttributeSearchResult result : results) {
      ...
      Map<String, List<SzSearchFeatureScore>> featureScores = result.getFeatureScores();
  ...
  public static void validateLoadRecordResponse(
    ...
    try {
      ...
      if (withInfo) {
        ...
        // check the interesting entities
